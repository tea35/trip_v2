...
# trip_linksテーブル作成スクリプト
# このスクリプトを実行してデータベースに変更を適用してください

echo "Creating trip_links table..."

# Supabase CLIを使用する場合
# supabase db reset
# または、SQL直接実行する場合は以下のSQLをSupabase Dashboardで実行してください
...
echo "SQLを上記の内容でSupabase Dashboardのクエリエディタで実行してください。"
